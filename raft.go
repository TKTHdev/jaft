...
func (cm *ConsensusModule) electionTimeout() time.Duration {
	...
	if len(os.Getenv("RAFT_FORCE_MORE_REELECTION")) > 0 && rand.Intn(3) == 0 {
		return time.Duration(150) *50* time.Millisecond
	} else {
		return time.Duration(150+rand.Intn(150)) *50* time.Millisecond
	}
}
...
func (cm *ConsensusModule) runElectionTimer() {
	...
	ticker := time.NewTicker(10 *50* time.Millisecond)
	defer ticker.Stop()
	for {
		<-ticker.C
...
func (cm *ConsensusModule) startLeader() {
	...
	go func() {
		ticker := time.NewTicker(50 *50* time.Millisecond)
		defer ticker.Stop()
